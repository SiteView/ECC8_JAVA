...
public class MonitorLoggers implements Runnable {
	...
	public MonitorLoggers(String server, Integer port, String userName,
		...
		Debug.logInfo("User < " + userName+" > login successfully!",module);
	...
	public void run() {
		...
		new PacketTypeFilter(Message.class), new FromContainsFilter(from + "@" + server));
		...
		while (true) {
			...
			if (packet != null) {// data arrived
				...
				if (packet instanceof Message) {
					...
					List<Map<String, String>> batchData = (List<Map<String, String>>) ObjectTransformation.SToO(msg.getBody());
					...
					for (Map<String, String> data : batchData) {
						...
						context.put("measurement", data.get("MonitorStatus") + " "+data.get("MonitorStr"));
