...
//
	...
	public String getReturnValue(String key, int index)
	{
		...
		if(m_fmap.containsKey(mkey))
		{
			...
			if (key.equals("MonitorName")&&str.equals("")) {
				String monitorNameAndCondition = OfbizCall.getMonitorNameAndCondition(m_node.getSvId());
	...
	public String getPropertyValue(String key)
	{
		...
		if(m_fmap.containsKey(m_node.getSvId()))
		{
			...
			if(rdata!=null && rdata.containsKey(key))
				str= rdata.get(key);
			if (key.equals("MonitorName")&&str==null) {
				String monitorNameAndCondition = OfbizCall.getMonitorNameAndCondition(m_node.getSvId());
				if (!monitorNameAndCondition.equals("")) {
					str=  monitorNameAndCondition.substring(0,monitorNameAndCondition.indexOf("#"));
				}
			}if (key.equals("errorCondition")&&str==null) {
				String monitorNameAndCondition = OfbizCall.getMonitorNameAndCondition(m_node.getSvId());
				if (!monitorNameAndCondition.equals("")) {
					str=  monitorNameAndCondition.substring(monitorNameAndCondition.indexOf("#")+1,monitorNameAndCondition.length());
				}
			}
		}
		return str;
