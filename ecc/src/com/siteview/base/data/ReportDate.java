...
//						double errorPercent = Double.parseDouble(propertyValue.get("okPercent"));
	...
	public String getReturnValue(String nodeid, String key, int index) {
		...
		if (m_fmap.containsKey(mkey)) {
			...
			if (rdata != null && rdata.containsKey(key))
				str = rdata.get(key);
			if (key.equals("MonitorName")&&str.equals("")) {
				String monitorNameAndCondition = OfbizCall.getMonitorNameAndCondition(nodeid);
				if (!monitorNameAndCondition.equals("")) {
					str=  monitorNameAndCondition.substring(0,monitorNameAndCondition.indexOf("#"));
				}
			}
		}
		return str;
	...
	public String getPropertyValue(String nodeid, String key) {
		...
		if (m_fmap == null)
			...
			Map<String, String> rdata = m_fmap.get(nodeid);
			if (rdata != null )
				retStr = rdata.get(key);
			if (retStr==null) {
				if (key.equals("MonitorName")) {
					String monitorNameAndCondition = OfbizCall.getMonitorNameAndCondition(nodeid);
					if (!monitorNameAndCondition.equals("")) {
						retStr=  monitorNameAndCondition.substring(0,monitorNameAndCondition.indexOf("#"));
					}
				}if (key.equals("errorCondition")) {
					String monitorNameAndCondition = OfbizCall.getMonitorNameAndCondition(nodeid);
					if (!monitorNameAndCondition.equals("")) {
						retStr=  monitorNameAndCondition.substring(monitorNameAndCondition.indexOf("#")+1,monitorNameAndCondition.length());
					}
				}
			}
