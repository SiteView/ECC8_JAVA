...
public class Jsvapi
{
	...
	private static ScasvapiMethodPrx slicescasvapi = null;
...
//			ic = Ice.Util.initialize(args);
			...
			.stringToProxy("scasvapimethod:default -p 10000 -h 127.0.0.1");

			slicescasvapi = ScasvapiMethodPrxHelper.checkedCast(base);
			if (slicescasvapi == null)
...
//		}
	...
	public void setServerAddr(String path)
	{
		...
		//configFile = "E:\\IceWorkspace\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\ecc" + sep + "svapi.ini";
...
//		DisplayUtilMapInMap(fmap);
	 ...
	 * 
	 */
	public boolean SwigGetUnivData(Map<String, Map<String, String>> fmap, Map<String, String> inwhat, StringBuilder estr)
	{
		...
		if (gocache)
			return SwigGetUnivData_Cache(fmap, inwhat, estr);
	 ...
	 * 
	 */
	public boolean SwigSubmitUnivData(Map<String, Map<String, String>> fmap, Map<String, String> inwhat, StringBuilder estr)
	{
		StringMap smap = UtilMapToStringMap(inwhat);
	 ...
	 * 
	 */
	public boolean SwigGetForestData(List<Map<String, String>> vmap, Map<String, String> inwhat, StringBuilder estr)
	{
		StringMap smap = UtilMapToStringMap(inwhat);
...
//		MyString mestr = new MyString();
		...
		SForestMapHolder s_fmap = new SForestMapHolder();
		StringHolder s_estr = new StringHolder();
		
//		boolean ret = SwigSvapi.swig_GetUnivData(fsmap, smap, mestr);
		boolean ret = slicescasvapi.SliceGetUnivData(inwhat, s_fmap, s_estr);
		
//		ForestMapToUtilMapInMap(fmap, fsmap);
		estr.delete(0, estr.length());
//		estr.append(mestr.getStr());
		//fmap = s_fmap.value;
		fmap.putAll(s_fmap.value);
		estr.append(s_estr.value);
...
//		MyString mestr = new MyString();
		...
		SForestMapHolder s_fmap = new SForestMapHolder();
		//s_fmap.value.putAll(fmap);
		
		StringHolder s_estr = new StringHolder();
		
//		boolean ret = SwigSvapi.swig_SubmitUnivData(fsmap, smap, mestr);
		boolean ret = slicescasvapi.SliceSubmitUnivData(inwhat, fmap, s_fmap, s_estr);
...
//		MyString mestr = new MyString();
		...
		SForestListHolder s_fmap = new SForestListHolder();
		StringHolder s_estr = new StringHolder();
		
//		boolean ret = SwigSvapi.swig_GetForestData(fvec, smap, mestr);
		boolean ret = slicescasvapi.SliceGetForestData(inwhat, s_fmap, s_estr);		
		
//		ForestArrayListToUtilMapInArrayList(vmap, fvec);
		estr.delete(0, estr.length());
//		estr.append(mestr.getStr());
		
		for(int i = 0; i < s_fmap.value.length; i++)
		{
			vmap.add(s_fmap.value[i]);
		}
	...
	private boolean SwigGetUnivData_Cache(Map<String, Map<String, String>> fmap, Map<String, String> inwhat, StringBuilder estr)
	{
...
//					estr.append(mestr.getStr());
					...
					SForestMapHolder s_fmap = new SForestMapHolder();
					StringHolder s_estr = new StringHolder();			
					ret = slicescasvapi.SliceGetUnivData(inwhat, s_fmap, s_estr);
					
					estr.delete(0, estr.length());
					tfmap.putAll(s_fmap.value);
					estr.append(s_estr.value);					
				} catch (Exception e)
				{
					ret = false;
