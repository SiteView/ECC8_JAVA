...
public class AlertRuleView extends AbstractWindow {
	...
	private static String SnmpTrap_TargetUrl = "/main/alert/snmptrapalert.zul";
	...
	public Menuitem getAddSnmpTrapMenuitem() {
		return (Menuitem) BaseTools.getComponentById(this, "addSnmpTrapAlert");
	}
	...
	class AddSnmpTrapClickListener implements org.zkoss.zk.ui.event.EventListener {
		AlertRuleView view = null;

		public AddSnmpTrapClickListener(AlertRuleView view) {
			this.view = view;
		}
		...
		public void onEvent(Event event) throws Exception {
			try {
				final Window win = (Window) Executions.createComponents(SnmpTrap_TargetUrl, null, null);
				win.setVariable(COMMAND_VAR, CMD_ADD, true);
				win.doModal();
				this.view.init();
			} catch (Exception e) {
...
//					Toolkit.getToolkit().setCookie("alert_rul_imfomation", basealert.getId(), 99999999);
	...
	public void onCreate() throws Exception {
		try {
			...
			this.getAddSnmpTrapMenuitem().addEventListener(Events.ON_CLICK, new AddSnmpTrapClickListener(this));
			...
			this.getAddSnmpTrapMenuitem().setImage(AlertType.SnmpTrapAlert.getImage());
